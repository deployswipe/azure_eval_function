...
Foundation of the swipe deploy system, the first piece of code to bootstrap your swipedeploy system.

Used to all the other things in the azure cloud platform.

Simple azure servless function that runs the eval or any other code in python.

Deployed using ansible inside of docker.

This function is open source and pull requests are welcome to improve on it, we are going to port it over to other cloud platforms as well.

The first goal is that it should also create a simple one click installer for you to deploy it to your cloud. Seconds goal is to hook it up the the app and gui, 
finally to provision the cloud for you as well.

# Concepts

Here are a list of concepts to wrap your head around before getting started.

## Azure

The azure serverless function is a managed server that runs your function in a pool of servers that automatically scale. 
https://azure.microsoft.com/en-us/services/functions/


### App Service Resource

The function has a source code repository and a url to access it. This App Service is the actual cpu allocation that runs your code. 
...
The subscription is the billing address that the service runs on. Make sure you dont leave things running, because even if you dont use them you will be billed. 

### Resource Group

All of the resources are grouped together in a resource group. These are just for organization and accounting.
...
Install my fork of the ansible azure collection via the requirements file, which only works with the latest unreleased version of ansible 
https://github.com/ansible/ansible/pull/69154

`ansible-galaxy collection install -r requirements.yml`

A workaround is to install the normal version and then overwrite it:

from where you checked out the https://github.com/deployswipe/ansible-collections-azure.git code , run this 

```
rm -rf ~/.ansible/collections/ansible_collections/azure/azcollection/plugins
ln -s -f `pwd`/plugins/ ~/.ansible/collections/ansible_collections/azure/azcollection/plugins
```
...
Just start xming with no access control, see the `config.xlaunch` file and run `login.sh`


### Details

If you are running in wsl you need to install xming, and use the xlaunch tool without access control to get the display to show on windows,

In powershell run `choco install xming` to install xming and then
double click on `config.xlaunch` to start xwindows.


Now, in bash, what the `login.sh` script does is this,
greps out your ip address from the autogenerated resolve.conf and then sets it to your display.

	export MYIP=`grep nameserver /etc/resolv.conf | cut '-d ' -f2`
	export DISPLAY="${MYIP}:0.0"

Setup the azure functions for debian package
...
Deployment options are needed
https://github.com/VSChina/ansible-issue-tracker/issues/178
